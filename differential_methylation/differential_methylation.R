...
sample_data <- read_csv("../2017-9059-1 Sample Sheet.csv")
sample_data_split <- str_split_fixed(sample.data$`External Sample ID`, "_", 3)
...
sample_data_final <- select(sample_data, barcode, Cell_Type, Replicate, Sample_Name)
...
rnb_narm_bmiq <- rnb.execute.na.removal.fix(rnb_norm_bmiq, 0)$dataset #Remove probes with missing values
...
#Remove probes with missing values and replace with rows of NAs
...
dnam_plot$Cell_Type %<>% factor(levels = c("ES", "NPC", "OPC(DIV180)", "OPC(DIV210)", "OPC(DIV240)", "ASTRO(DIV180)", "ASTRO(DIV210)", "ASTRO(DIV240)"))

p <- ggplot(dnam_plot, aes(Cell_Type, DNAmAge, col = Cell_Type)) + 
    geom_point() + 
    theme_bw() + 
    theme(panel.grid.major = element_blank(), 
          panel.grid.minor = element_blank(),
...
kw_statistic <- kruskal.test(DNAmAge ~ Cell_Type, dnam_plot) %>% tidy
dunn_statistic <- posthoc.kruskal.dunn.test(DNAmAge ~ Cell_Type, dnam_plot, p.adjust.method = "fdr")

#Bayesian ANOVA
dnam_bf <- anovaBF(DNAmAge ~ Cell_Type, data.frame(dnam_plot)) 
dnam_posterior <- posterior(dnam_bf, iterations = 100000) %>% 
    data.frame %>% select(mu, matches("^Cell_Type")) 
colnames(dnam_posterior) %<>% str_replace_all("Cell_Type\\.", "") %>% str_replace_all("\\.$", "") %>% str_replace_all("\\.", "_")
dnam_posterior_mu <- dnam_posterior$mu
dnam_posterior_predictive <- sweep(select(dnam_posterior, -mu), 1, dnam_posterior_mu, "+")
dnam_posterior_plot <- gather(dnam_posterior_predictive, Cell_Type, Estimate)
dnam_posterior_plot$Cell_Type %<>% factor(levels = c("ES", "NPC", "OPC_DIV180", "OPC_DIV210", "OPC_DIV240", "ASTRO_DIV180", "ASTRO_DIV210", "ASTRO_DIV240"))

#Plot posterior estimate of methylation age for each cell type
p <- ggplot(dnam_posterior_plot, aes(x = Cell_Type, y = Estimate, fill = Cell_Type)) + 
...
#Annotate CpG Islands - this is kind of complicated!
...
#Gene Annotation - retrieve information from BioMart
...
#Reformat cell types to be easier to work with
...
#Run differential methylation with the specialized contrasts
...
#Extract and reformat comparisons
comparisons <- get.comparisons(diffmeth)
comparisons_format <- str_replace(comparisons, " \\(.*$", "") %>% 
    str_replace("\\.", "") %>%
    str_replace_all(" ", "_")

#Combine tables for individual contrasts into one table
